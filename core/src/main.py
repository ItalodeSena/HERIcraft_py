...
parser = argparse.ArgumentParser(
    description="Generate cities from real life in Minecraft using Python"
)
...
def run():
    ...
    # # check "http://127.0.0.1:5000" is up and running
    ...
    # except requests.exceptions.RequestException as e:
    #     print(f"Error: Unable to connect to the server at {url}. {e}")
    #     sys.exit(1)


    # r"0-arnis-debug-raw_data.json"
    raw_data_file = f"data/osm_{'_'.join(map(str, bbox))}_raw_data.json"
    ...
    if os.path.exists(raw_data_file):
        ...
        print(f"[INFO] \033[91m [ONLINE] \033[0m Downloading data from OpenStreetMap")
    ...
    for i in tqdm(imgarray, desc="Generating city", total=len(imgarray), unit="row"):
        ...
        for j in i:
            ...
            # v4 : setting the terrain here
            # index_of_i = np.where(imgarray == i)
            # index_of_j = np.where(i == j)
            # print(f"index_of_i: {index_of_i}")
            # print(f"index_of_j: {index_of_j}")
            # print(f"imgTerrain[index_of_i][index_of_j]: {imgTerrain[index_of_i][index_of_j]}")
            # h = imgTerrain[index_of_i][index_of_j]
            # try:
            #     # print(f"[x]={x}, [z]={z}, y={imgTerrain[z][x]}")
            #     # setBlockwithElevation(stone, x, 0, z, imgTerrain[z][x])
            #     h = imgTerrain[z][x]
            # except:
            #     print(f"Error: x: {x}, z: {z}")
            #     h = 0   
            h=0
            fillBlocks(stone, x, 0, z, x, h, z)
            setBlock(dirt, x, h+1, z) # This is the terrain?
            if j == 0:  # Ground
                setBlock(light_gray_concrete, x, h+1, z)              
            elif j == 10:  # Street (road)
                setBlock(black_concrete, x, h+1, z)
                setBlock(air, x, h+2, z)
            elif j == 11:  # Footway
                setBlock(gray_concrete, x, h+1, z)
                setBlock(air, x, h+2, z)
            elif j == 12:  # Natural path
                setBlock(cobblestone, x, h+1, z)
            elif j == 14:  # Railway
                setBlock(iron_block, x, h+2, z)
            elif j == 20:  # Parking
                setBlock(gray_concrete, x, h+1, z)
            elif j == 21:  # Fountain border
                setBlock(light_gray_concrete, x, h+2, z)
                setBlock(white_concrete, x, h+1, z)
            elif j >= 22 and j <= 24:  # Fence
                if str(j)[-1] == "2" or int(str(j[0])[-1]) == 2:
                    setBlock(cobblestone_wall, x, h+2, z)
                else:
                    fillBlocks(cobblestone, x, h+2, z, x, h+int(str(j[0])[-1]), z)

                setBlock(grass_block, x, h+1, z)
            elif j == 30:  # Meadow
                setBlock(grass_block, x, h+1, z)
                randomChoice = randint(0, 2)
                if randomChoice == 0 or randomChoice == 1:
                    setBlock(grass, x, h+2, z)
            elif j == 31:  # Farmland
                randomChoice = randint(0, 16)
                if randomChoice == 0:
                    setBlock(water, x, h+1, z)
                elif randomChoice >= 1 and randomChoice <= 4:
                    if args.debug: 
                        print("[DEBUG] Adding a tree to farmland...")
                    addTree(x, z)
                else:
                    setBlock(farmland, x, h+1, z)
                    randomChoice = randint(0, 2)
                    if randomChoice == 0:
                        setBlock(wheat, x, h+2, z)
                    elif randomChoice == 1:
                        setBlock(carrots, x, h+2, z)
                    else:
                        setBlock(potatoes, x, h+2, z)
            elif j == 32:  # Forest
                setBlock(grass_block, x, h+1, z)
                ...
                setBlock(podzol, x, h+1, z)
                randomChoice = randint(0, 100)
                if randomChoice == 0:
                    setBlock(cobblestone, x - 1, h+2, z)
                    setBlock(stone_brick_slab, x - 1, h+3, z)
                    setBlock(stone_brick_slab, x, h+2, z)
                    setBlock(stone_brick_slab, x + 1, h+2, z)
                elif randomChoice == 1:
                    setBlock(cobblestone, x, h+2, z - 1)
                    setBlock(stone_brick_slab, x, h+3, z - 1)
                    setBlock(stone_brick_slab, x, h+2, z)
                    setBlock(stone_brick_slab, x, h+2, z + 1)
                elif randomChoice == 2 or randomChoice == 3:
                    setBlock(red_flower, x, h+2, z)
            elif j == 34:  # Beach
                setBlock(sand, x, h+1, z)
            elif j == 35:  # Wetland
                randomChoice = randint(0, 2)
                if randomChoice == 0:
                    setBlock(grass_block, x, h+1, z)
                ...
                setBlock(green_stained_hardened_clay, x, h+1, z)
            elif j == 37:  # Swimming pool
                setBlock(water, x, h+1, z)
                setBlock(white_concrete, x, h, z)
            elif j == 38:  # Water
                setBlock(water, x, h+1, z)
            elif j == 13:  # Bridge
                addBridge(x, h, z)
                ...
                fillBlocks(white_concrete, x, h+1, z, x, h+building_height, z)
                ...
                # if doorIncrement == 25:
                #     fillBlocks(white_stained_glass, x, h+4, z, x, building_height, z)
                #     setBlock(white_concrete, x, h+1, z)
                #     setBlock(dark_oak_door_lower, x, h+2, z)
                #     setBlock(dark_oak_door_upper, x, h+3, z)
                #     doorIncrement = 0
                # else:
                fillBlocks(white_concrete, x, h+1, z, x, h+2, z)
                fillBlocks(white_stained_glass, x, h+3, z, x, h+building_height, z)
                # doorIncrement += 1
                setBlock(white_concrete, x, h+ building_height + 1, z)
            elif j >= 70 and j <= 79:  # House interior
                if j >= 70:
                    setBlock(white_concrete, x, h+5, z)
                    if j >= 71:
                        setBlock(white_concrete, x, h+8, z)
                        if j >= 72:
                            setBlock(white_concrete, x, h+11, z)
                            if j >= 73:
                                setBlock(white_concrete, x, h+14, z)
                                if j >= 74:
...
def addBridge(x, h, z):
    setBlock(light_gray_concrete, x, h+2, z)
    setBlock(light_gray_concrete, x - 1, h+2, z - 1)
    setBlock(light_gray_concrete, x + 1, h+2, z - 1)
    setBlock(light_gray_concrete, x + 1, h+2, z + 1)
    setBlock(light_gray_concrete, x - 1, h+2, z + 1)
...
def addTree(x, z):
    # replace with oak_sapling
    oak_sapling = anvil.Block("minecraft:oak_sapling")
    setBlock(oak_sapling, x, 2, z)
